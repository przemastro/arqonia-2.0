...
keyStore=${appPrefix}_https_it.jks
alias=${appPrefix}_https_it
password=pass123
csrFile=ca/${appPrefix}_https_it.csr

caCertificate=ca/${appPrefix}_ca_certificate_it.pem
caKey=ca/private/${appPrefix}_ca_key_it.pem

certificateName=ca/${appPrefix}_https_certificate_it.cer
...
keytool -keystore ${keyStore} -genkey -dname "CN=localhost, OU=Arqonia, O=Arqonia, L=Poznan, ST=Wielkopolska, C=PL" \
...
openssl req -new -x509 -days 3650 -extensions v3_ca \
    -subj "/C=PL/ST=Wielkopolska/L=Poznan/O=Arqonia/OU=Arqonia/CN=localhost" \
...
# import CA certificate to java 'cacerts' key store (needed for self-signed certificates during integration tests)
sudo keytool -noprompt -importcert -file ${caCertificate} -alias ${alias} \
