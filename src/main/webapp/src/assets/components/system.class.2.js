  ...
  'create' : function(val, withSolid) {
    ...
    if(this.particleGeo !== null) {
      ...
      if(val.infos != undefined && this.particleInfos[idSys]) {
        ...
        if(val.cat != undefined) ArqoniaMap.addObjToCategories(indexParticle,val.cat);
        return;
      }
      ...
      if(val.cat != undefined && val.cat[0] != undefined && ArqoniaMap.colors[val.cat[0]] != undefined) {
        this.particleColor[this.count] = ArqoniaMap.colors[val.cat[0]];
      } else {
        this.particleColor[this.count] = new THREE.Color(ArqoniaMap.systemColor);
      }

      //-- If system got some categories, add it to cat list and save his main color

      if(val.cat != undefined) {
        ArqoniaMap.addObjToCategories(this.count,val.cat);
    ...
    if(withSolid) {
      ...
      var mat = ArqoniaMap.material.glow_1;
